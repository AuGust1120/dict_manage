...
const updateRecord = async (id, newData) => {
  ...
  //newData.update_time = new Intl.DateTimeFormat('en-US', options).format(date);
  console.log('zhongguoshijian;',newData.update_time);
...
const updateDict = async (id, newData) => {
  ...
  update_time = DATEADD(HOUR, +8, @update_time), deleted = @deleted,sort = @sort,css_class = @css_class WHERE id = @id;`;
...
// 函数：根据分页参数和查询条件获取字典数据记录
const queryPageDict = async (pageN, pageS, name, label, status, type,children) => {
  const offset = (pageN - 1) * pageS;
  const limit = pageS;
  console.log(type);
  let sqlQuery = `SELECT id,label,value,sort,param_id,status,color_type,css_class,remark,DATEADD(HOUR, -8, create_time) AS create_time FROM system_dict_data WHERE deleted = 0 AND dict_type ='${type}'`;
  ...
  let childrenQuery =null;
  const childrenResult =null;
  const params = [];
...
JOIN 
    system_dict_data s2
ON 
    s1.param_id = s2.label;
`;
childrenResult= await pool
  .request()
  .query(childrenQuery);
  }else{
...
app.post("/searchDataDict", async (req, res) => {
  const data = req.body;
  try {
    const { results, totalCount, parentResult, allParentResult,childrenResult} =
      await queryPageDict(
        data.pageNo,
        data.pageSize,
        data.name,
    ...
    res.json({ results, totalCount, parentResult, allParentResult,childrenResult});
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "An error occurred" });
  }
