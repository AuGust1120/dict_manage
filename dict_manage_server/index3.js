...
app.post("/createDict", (req, res) => {
  ...
  // 构建 SQL 查询语句和参数
  const sqlQuery = `INSERT INTO system_dict_type (name, type, status, update_time) VALUES ('${data.name}','${data.type}','${data.status}','${data.update_time}')`
  const params = {
    name: data.name,
    type: data.type,
    status: data.status,
...
app.post("/createDictData", (req, res) => {
  ...
  const sqlQuery = `INSERT INTO system_dict_data (value, label, status, update_time) VALUES ('${data.name}','${data.label}','${data.status}','${data.update_time}')`
  const params = {
    name: data.name,
    label: data.label,
    status: data.status,
...
const queryPageRecords = async (pageN, pageS, name, type, status, create_time) => {
  ...
  if (status) {
    ...
    countSql += ` AND status LIKE '%${status}%'`;
  }

    if (create_time[0]) {
      sqlQuery += ` AND '${create_time[0]}'>=create_time AND'${create_time[1]}'<=update_time`;
      countSql += ` AND '${create_time[0]}'>=create_time AND'${create_time[1]}'<=update_time`;
    }
...
const queryPageDict = async (pageN, pageS, name, label, status,type) => {
  ...
  if (label) {
    sqlQuery += ` AND type LIKE '%${label}%'`;
    countSql += ` AND type LIKE '%${label}%'`;
  }
  ...
  const parentResult = await pool
    ...
    .input("label", sql.VarChar, label)
    .input("status", sql.Int, status).input("offset", sql.Int, offset)
    .input("limit", sql.Int, limit).query(parentQuery);
  const allParentResult = await pool
    .request().input("name", sql.VarChar, name)
    .input("label", sql.VarChar, label)
    .input("status", sql.Int, status).input("offset", sql.Int, offset)
    .input("limit", sql.Int, limit).query(allParentQuery);
  // 执行记录数量查询
  const countResult = await pool
    .request()
    .input("name", sql.VarChar, name)
  ...
  const totalCount = countResult.recordset[0].totalCount;
  return { results: result.recordset, totalCount,parentResult,allParentResult};
};
...
app.post("/searchDataDict", async (req, res) => {
  ...
  console.log(data);
  try {
    const { results, totalCount,parentResult,allParentResult } = await queryPageDict(
      data.pageNo,
      data.pageSize,
      data.name,
      data.label,
